...
def find_gps_and_slam_files(folder_path):
    """
    Finds:
    - GPS file: starts with 'gps_data_' and ends with '.txt'
    - SLAM file: either 'poses_tum_converted.txt' OR starts with 'slam_poses_' and ends with '.txt'
    """
    gps_file = None
    slam_file = None
    ...
    for fname in os.listdir(folder_path):
        ...
        # Find SLAM file (two possible names)
        if fname == "poses_tum_converted.txt":
            slam_file = os.path.join(folder_path, fname)
        elif fname.startswith("slam_poses_") and fname.endswith(".txt"):
            slam_file = os.path.join(folder_path, fname)

    if not gps_file:
        raise FileNotFoundError("No GPS file starting with 'gps_data_' found.")
    if not slam_file:
        raise FileNotFoundError("No SLAM file found. Expected 'poses_tum_converted.txt' or 'slam_poses_*.txt'.")
...
def main_pipeline(folder_path, plot_steps=True):
    print(f"Processing folder: {folder_path}")
    os.makedirs(folder_path, exist_ok=True)

    try:
        gps_file, slam_file = find_gps_and_slam_files(folder_path)
    except FileNotFoundError as e:
        print(f"Error: {e}")
        return

    output_file = os.path.join(folder_path, "slam_geo_referenced.txt")
    ...
    # Plot 1: Raw
    if plot_steps:
        plot_raw_trajectories(gps_lats, gps_lons, slam_xs, slam_ys, folder_path)
    ...
    if plot_steps:
        plot_converted_gps(gps_eastings, gps_northings, folder_path)
    ...
    save_alignment_parameters(scale, R, T, folder_path)
    ...
    # Plot 3: Final
    if plot_steps:
        plot_only_georeferenced_slam(aligned_slam, gps_hk1980, folder_path)

    # Maps
    create_gps_accuracy_map(gps_file, os.path.join(folder_path, "gps.html"))
    create_slam_map(output_file, os.path.join(folder_path, "slam_geo_referenced.html"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process folder: auto-detect GPS & SLAM → full pipeline")
    parser.add_argument("folder", help="Path to folder with gps_data_*.txt and poses_tum_converted.txt")
    ...
    main_pipeline(args.folder, not args.no_plots)