...
def main_pipeline(folder_path, plot_steps=True):
    ...
    # Plot pre-aligned for debugging (optional)
    # plt.figure(); plt.plot(gps_hk1980[:,0], gps_hk1980[:,1], 'b-', label='GPS')
    # plt.plot(slam_xy_pre[:,0], slam_xy_pre[:,1], 'r-', label='SLAM pre-aligned')
    # plt.legend(); plt.show()
    ...
    # Apply transformation to full SLAM trajectory (use pre-aligned version)
    ...
    # Find closest SLAM points to GPS (for error estimation)
...
if __name__ == "__main__":
    ...
    parser.add_argument("folder", help="Path to folder with gps_data_*.txt and slam/pose file")
